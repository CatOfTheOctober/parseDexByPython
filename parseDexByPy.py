...
static_fields_list =[]              #class中static field全部信息
...
def parseClassIdList():
    ...
    print("---***---class_ids_size---***---")
    print("类型数量：",class_ids_size)  
    ...
    for i in range(class_ids_size):
        ...
        class_data_item_ =parseClassDataItem(class_data_off_)
        class_item.class_data_off =class_data_item_

        static_valuse_off =readFileHexData(class_ids_off +0x20 *i +28 ,4)   #静态字段初始值列表在文件中的偏移值，如果此值为0则所有静态字段都被初始化为0或null。由"encoded_array_item"格式描述此初始值
        class_item.static_values_str =static_valuse_off

        classs_list.append(class_item)
    printClassDefList()
...
def parseClassDataItem(off):
    ...
    tmp =readUled128(tmp[1])
    direct_methods_size_ =tmp[0]
    class_data_item_.direct_methods_size =direct_methods_size_
    ...
    for static_fields_i in range(static_fields_size_):                #解析static_fields 结构：uleb128 field_idx_diff; uledb128 access_flags
        ...
        static_field_item.access_flags_str =access_flags

        static_fields_list.append(static_field_item)
    ...
    for instance_fields_i in range(instance_fields_size_):
        ...
        instance_field_item.field_diff_str =field_diff
        instance_field_item.access_flags_str =access_flags

        instance_fields_list.append(instance_field_item)
    ...
    for direct_methods_i in range(direct_methods_size_):
        ...
        access_flags =tmp[0]
        tmp =readUled128(tmp[1])
        code_off =tmp[0]            #code在文件中的偏移量
        code_item_ =parseCodeItem(code_off)

        direct_method_item.method_diff_str =method_diff
        direct_method_item.access_flags_str =access_flags
        direct_method_item.code_data =code_item_

        direct_methods_list.append(direct_method_item)
    ...
    for virtual_methods_i in range(virtual_methods_size_):
        ...
        virtual_method_itme.method_diff_str =method_diff
        virtual_method_itme.access_flags_str =access_flags
        virtual_method_itme.code_data =code_item_

        virtual_methods_list.append(virtual_method_itme)
    if (len(virtual_methods_list) !=0):
        class_data_item_.virtual_methods =virtual_method_itme
    
    return class_data_item_
    


'''
函数功能：解析code_item
函数参数：code_item在文件中的偏移
函数偏移：返回code_item结构
'''    
def parseCodeItem(off):
    code_item_ =code_item()

    register_size_ =readFileHexData(off ,2)
    ins_size_ =readFileHexData(off +2 ,2)
    outs_size_ =readFileHexData(off +4 ,2)
    tries_size_ =readFileHexData(off +6 ,2)
    debug_info_off_ =readFileHexData(off +8 ,4) #调试信息在文件中的偏移，如果 此值为0则不存在调试信息。此处不解析此项
    insns_size_ =readFileHexData(off +12 ,4)    #代码使用的指令数量，每个指令占用两个字节，指令字节数大小 = 2 *insns_size_
    
    insns_list_ =[]      #用于存放指令列表
    for i in range(insns_size_):
        dexFileMmap.seek(off +16 +2*i)
        insns_ =dexFileMmap.read(2)
        insns_list_.append(insns_)

    padding_ =None
    if ((tries_size_ !=0) and (insns_size_%2 !=0)):   #padding填充数据，只有当 tries_size!=0并且 insns_size_为奇数是存在
        dexFileMmap.seek(off +16 +2*ins_size_)
        padding_ =dexFileMmap.read(2)

    try_item_off_ =None
    if(tries_size_ !=0):                              #异常处理代码在文件中的偏移，该数据以"try_item"格式进行解析
        dexFileMmap.seek(off +16 +2*ins_size_ +2)
        try_item_off =dexFileMmap.read(4)
    
    code_item_.register_size =register_size_
    code_item_.ins_size =ins_size_
    code_item_.outs_size =outs_size_
    code_item_.tries_size =tries_size_
    code_item_.debug_info_off =debug_info_off_
    code_item_.insns_size =insns_size_
    code_item_.insns =insns_list_
    code_item_.padding =padding_
    code_item_.tries =try_item_off_
    code_item_.handleres =None
    return code_item_
    


'''
函数功能：打印class_def_item内容
函数参数：class_def的信息
...
def printClassDefList():
    for class_list_i in range(len(classs_list)):
        ...
        class_data_item_ =class_data_item()
        class_data_item_ =class_item.class_data_off 
        print("  static_fields_size", class_data_item_.static_fields_size)
        print("  instance_fields_size", class_data_item_.instance_fields_size)
        print("  direct_methods_size", class_data_item_.direct_methods_size)
        print("  virtual_methods_size", class_data_item_.virtual_methods_size)
        ...
        for direct_methods_i in range(class_data_item_.direct_methods_size):
            ...
            tmp_str =str()
            for i_print in range(len(method_item_.proto_str.parameters_str)):
                if( i_print==0):
            ...
            print("     register_size", code_item_.register_size)
            print("     ins_size", code_item_.ins_size)
            print("     outs_size", code_item_.outs_size)
            print("     tries_size", code_item_.tries_size)
...
'''
函数功能：读取文件中的数据
函数参数：off:在文件中的偏移;len:读取大小;list:使用的索引表(非必须)
函数返回：
'''
